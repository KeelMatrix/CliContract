...
The accepted invocation graph includes aliases at every command segment; a retained alias preserves old command paths and descendants. The adapter checks all supported type-pair lexical domains and rejects non-variadic item bounds, multiple variadic positional arguments, and variadic arguments that are not last. Global and command exit-code metadata is preserved and classified as a warning when changed. `x-*` extensions are outside the compatibility decision. Source and baseline file-read failures are exit `3`, while suppression/configuration and output-write failures are exit `2`.
