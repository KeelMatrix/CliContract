...
- Source aliases and alternative-source properties use the same empty-only presence rule: non-empty whitespace is preserved, while empty or duplicate aliases remain invalid. Structural command paths, normalized `--` option names, and global config file paths retain their grammar/non-whitespace constraints.
