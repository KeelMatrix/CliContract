...
- Every canonical command segment after `root` starts with an ASCII letter, matching the alpha.14 command-key parser; materialized ancestors are present in the flat root command list and missing ancestors are `group` nodes.
- Alpha.14 requires nonempty `info.title`, `info.binary`, `info.version`, license names, exit-code summaries, example content, install names, parameter names, and config-source paths. For source-shaped text carried into the manifest, whitespace-only values are nonempty data and are preserved verbatim; empty values remain invalid. A contact contains at least one representable name/email/URL field, and every install method contains a command or URL.
- Source aliases and alternative-source properties use the same empty-only presence rule: non-empty whitespace is preserved, while empty or duplicate aliases remain invalid. Option names are constructed as `--` plus the source name after `TrimStart('-')`; whitespace, Unicode, punctuation, and dash-only names are therefore preserved or normalize to `--` rather than rejected by a separate lexical rule. Global config file paths retain their non-whitespace-only constraint.
- Option types use the exact alpha.14 values (`string`, `number`, `integer`, or `boolean`); arguments may omit `type` but use the same exact values when present. Canonical baselines reject status/deprecation fields that alpha.14 cannot emit.
