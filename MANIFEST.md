...
- Alpha.14 requires nonempty `info.title`, `info.binary`, `info.version`, license names, exit-code summaries, example content, install names, parameter names, and config-source paths. A contact contains at least one representable name/email/URL field, and every install method contains a command or URL.
