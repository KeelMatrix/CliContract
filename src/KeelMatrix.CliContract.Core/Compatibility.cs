...
public static class CompatibilityAnalyzer
{
    ...
    private sealed class AcceptedDomain
    {
        ...
        public static AcceptedDomain Create(CanonicalParameter parameter) => new(TypedDomain.NormalizeType(parameter.Type), parameter.Choices.Length == 0 ? null : parameter.Choices.Select(choice => choice.Value).ToArray());
        ...
        private static bool Accepts(string type, JsonNode value) => TypedDomain.Accepts(type, value, allowNumericStrings: true);

        private static bool EquivalentValue(string type, JsonNode left, JsonNode right) => TypedDomain.Equivalent(type, left, right, allowNumericStrings: true);
    }
