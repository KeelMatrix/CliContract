...
The selected diagnostic catalog and classification contract are in [`COMPATIBILITY-RULES.md`](COMPATIBILITY-RULES.md); the complete role-aware error taxonomy is [`docs/ERROR-TAXONOMY.md`](docs/ERROR-TAXONOMY.md). The versioned canonical manifest (schema version `2`) is described in [`MANIFEST.md`](MANIFEST.md). Global options remain separate from root-local options and are inherited by every command; the manifest also materializes missing command ancestors as derived groups so compatibility is based on the callable surface.
