...
| Option types are exact alpha.14 values and names use canonical `--name` form; aliases are nonempty, duplicate-free normalized collections and preserve whitespace | `ValidateParameter` and `ValidateOptionCollection` | `CanonicalManifestReaderRequiresExactAlpha14ParameterTypes`, `DuplicateSourceAliasesRemainRejected`, and hostile `invalid-option-name` / `invalid-option-alias` |
