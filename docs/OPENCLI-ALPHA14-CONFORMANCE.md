...
| --- | --- | --- |
| `info.title`, `info.binary`, and `info.version` are required and nonempty | `CanonicalInvariantValidator.Validate` | `CanonicalManifestReaderRejectsEveryHostileSourceImpossibleState` / `missing-info-title` |
...
| Each install method has a nonempty `name` and a `command` or `url` | `CanonicalInvariantValidator.ValidateInfo` | `CanonicalManifestReaderRejectsEveryInstallPresenceVariant` / `install-without-command-or-url`; installed counterpart |
| License name and example content are required and nonempty | `CanonicalInvariantValidator.ValidateInfo` and `ValidateExamples` | hostile reader cases `license-empty-name` and `empty-example-content` |
...
| Exit-code values are unique, statuses are from alpha.14, summaries are nonempty, and values are sorted | `ValidateExitCodes` | hostile `duplicate-global-exit-code`, `duplicate-command-exit-code`, and `invalid-exit-status` |
| `$ENV`/`$FILE` sources use supported types and nonempty properties; `$FILE` needs global file configuration | `ValidateSources` | hostile `invalid-source-type`, `empty-source-property`, and `file-source-without-config` |
...
| Option types are exact alpha.14 values and names use canonical `--name` form; aliases are normalized collections | `ValidateParameter` and `ValidateOptionCollection` | `CanonicalManifestReaderRequiresExactAlpha14ParameterTypes` and hostile `invalid-option-name` / `invalid-option-alias` |
