...
Canonical baselines are accepted only when they are states the supported alpha.14 normalizer can produce. The reader rejects duplicate global file-source formats or exit-code values, invalid command paths or hierarchy, invalid option names and aliases, impossible argument/option arity or source combinations, contradictory choices, unsupported status/deprecation values, and global/local accepted-name collisions before comparison. These malformed-but-valid JSON baselines are baseline diagnostics with exit `3`, never `UNEXPECTED_ERROR` (`4`).
