...
Canonical baselines are accepted only when they are states the supported alpha.14 normalizer can produce. The reader rejects missing required metadata, empty license/example/install fields, contact/install presence violations, non-letter command segments or invalid hierarchy, duplicate global file-source formats or exit-code values, empty global-config wrappers, invalid option names/types and aliases, requiredness-inconsistent non-variadic arity, invalid variadic required/min/max combinations, invalid positional ordering or variadic placement, impossible source combinations, reversed scalar choices or contradictory choice domains, unsupported status/deprecation values, and global/local accepted-name collisions before comparison. These malformed-but-valid JSON baselines are baseline diagnostics with exit `3`, never `UNEXPECTED_ERROR` (`4`).
